...
# Dense weight
w_dense = torch.randn(N, K, device=device, dtype=dtype)
...
dense_latency = benchmark(lambda: torch.nn.functional.linear(x, w_dense))

############################################################
# Sparse benchmark
############################################################

sparse_latency = benchmark(lambda: torch.nn.functional.linear(x, w_sparse))

############################################################
# Verify correctness
############################################################

dense_result = torch.nn.functional.linear(x, w)
sparse_result = torch.nn.functional.linear(x, w_sparse)
